...
<!-- AUTO-GENERATED from shared/rules/forbidden-actions.md. Do not edit here; edit the source and run `python scripts/sync_rules.py`. -->
...
- **F-01 禁止在 04 契约冻结后推进下游时增删段落**：04 契约冻结后，下游（05-10）不得增删/合并/拆分段落或镜头、不得改写标题或口播、不得重新引入 `anti_hype_forbidden` 噱头词。（来源：`04-script-draft` / `07-video-assembly` 工作流）
...
- **F-05 禁止提交敏感信息**：API-Key、token、凭据等严禁进入任何提交与归档产物。（来源：项目通用安全规范）
...
- **F-16 禁止噱头/夸张卖点**：标题与口播不得使用当期 `anti_hype_forbidden` 黑名单词（「一键出片 / 百倍效率 / 碾压」等），对照轴不得写成「手写 vs 自动 / 省人力」。（来源：`04-script-draft` / `voice-style.md`）
